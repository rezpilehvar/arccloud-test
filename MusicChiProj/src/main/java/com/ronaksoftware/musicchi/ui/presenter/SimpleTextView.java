...
public class SimpleTextView extends View implements Drawable.Callback {
    ...
    private int drawablePadding = DisplayUtility.dp(4);
    ...
    public void setTextSize(int size) {
        int newSize = DisplayUtility.dp(size);
    ...
    private boolean createLayout(int width) {
        if (text != null) {
            try {
                ...
                layout = new StaticLayout(string, 0, string.length(), textPaint, scrollNonFitText ? DisplayUtility.dp(2000) : width + DisplayUtility.dp(8), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                calcOffset(width);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (lastWidth != DisplayUtility.displaySize.x) {
            lastWidth = DisplayUtility.displaySize.x;
            scrollingOffset = 0;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int nextScrollX = totalWidth + DisplayUtility.dp(DIST_BETWEEN_SCROLLING_TEXT);
        ...
        if (layout != null) {
            ...
            if (scrollNonFitText && (textDoesNotFit || scrollingOffset != 0)) {
                if (scrollingOffset < DisplayUtility.dp(10)) {
                    fadeDrawable.setAlpha((int) (255 * (scrollingOffset / DisplayUtility.dp(10))));
                } else if (scrollingOffset > totalWidth + DisplayUtility.dp(DIST_BETWEEN_SCROLLING_TEXT) - DisplayUtility.dp(10)) {
                    float dist = scrollingOffset - (totalWidth + DisplayUtility.dp(DIST_BETWEEN_SCROLLING_TEXT) - DisplayUtility.dp(10));
                    fadeDrawable.setAlpha((int) (255 * (1.0f - dist / DisplayUtility.dp(10))));
                } else {
                    fadeDrawable.setAlpha(255);
                }
                fadeDrawable.setBounds(0, 0, DisplayUtility.dp(6), getMeasuredHeight());
                fadeDrawable.draw(canvas);

                fadeDrawableBack.setBounds(getMeasuredWidth() - DisplayUtility.dp(6), 0, getMeasuredWidth(), getMeasuredHeight());
    ...
    private void updateScrollAnimation() {
        ...
        if (currentScrollDelay > 0) {
            ...
            int totalDistance = totalWidth + DisplayUtility.dp(DIST_BETWEEN_SCROLLING_TEXT);
            float pixelsPerSecond;
            if (scrollingOffset < DisplayUtility.dp(SCROLL_SLOWDOWN_PX)) {
                pixelsPerSecond = PIXELS_PER_SECOND_SLOW + (PIXELS_PER_SECOND - PIXELS_PER_SECOND_SLOW) * (scrollingOffset / DisplayUtility.dp(SCROLL_SLOWDOWN_PX));
            } else if (scrollingOffset >= totalDistance - DisplayUtility.dp(SCROLL_SLOWDOWN_PX)) {
                float dist = scrollingOffset - (totalDistance - DisplayUtility.dp(SCROLL_SLOWDOWN_PX));
                pixelsPerSecond = PIXELS_PER_SECOND - (PIXELS_PER_SECOND - PIXELS_PER_SECOND_SLOW) * (dist / DisplayUtility.dp(SCROLL_SLOWDOWN_PX));
            } else {
                pixelsPerSecond = PIXELS_PER_SECOND;
            }
            scrollingOffset += dt / 1000.0f * DisplayUtility.dp(pixelsPerSecond);
