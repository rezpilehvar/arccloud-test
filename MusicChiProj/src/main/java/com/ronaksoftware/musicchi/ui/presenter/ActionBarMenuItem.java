...
import com.ronaksoftware.musicchi.ui.components.CloseProgressDrawable2;
import com.ronaksoftware.musicchi.ui.components.EditTextBoldCursor;
...
public class ActionBarMenuItem extends FrameLayout {
    ...
    public ActionBarMenuItem(Context context, ActionBarMenu menu, int backgroundColor, int iconColor, boolean text) {
        ...
        if (text) {
            ...
            textView.setPadding(DisplayUtility.dp(4), 0, DisplayUtility.dp(4), 0);
            if (iconColor != 0) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN) {
            if (longClickEnabled && hasSubMenu() && (popupWindow == null || popupWindow != null && !popupWindow.isShowing())) {
                ...
                Queues.runOnUIThread(showMenuRunnable, 200);
            }
    ...
    public TextView addSubItem(int id, CharSequence text) {
        ...
        textView.setPadding(DisplayUtility.dp(16), 0, DisplayUtility.dp(16), 0);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 16);
        textView.setMinWidth(DisplayUtility.dp(196));
        ...
        layoutParams.height = DisplayUtility.dp(48);
        ...
        textView.setOnClickListener(view -> {
            if (popupWindow != null && popupWindow.isShowing()) {
                ...
                if (!allowCloseAnimation) {
                    popupWindow.setAnimationStyle(R.style.Theme_MusicChi_PopupAnimation);
                }
                popupWindow.dismiss(allowCloseAnimation);
            }
            if (parentMenu != null) {
    ...
    public ActionBarMenuSubItem addSubItem(int id, int icon, CharSequence text) {
        ...
        cell.setMinimumWidth(DisplayUtility.dp(196));
        cell.setTag(id);
    ...
    public void toggleSubMenu() {
        ...
        if (showMenuRunnable != null) {
            Queues.cancelRunOnUIThread(showMenuRunnable);
        ...
        if (measurePopup) {
            popupLayout.measure(MeasureSpec.makeMeasureSpec(DisplayUtility.displaySize.x - DisplayUtility.dp(40), MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(DisplayUtility.displaySize.y, MeasureSpec.AT_MOST));
            measurePopup = false;
        }
    ...
    public boolean toggleSearch(boolean openKeyboard) {
        ...
        if (searchContainer.getVisibility() == VISIBLE) {
            if (openKeyboard) {
                DisplayUtility.hideKeyboard(searchField);
            }
            ...
            if (openKeyboard) {
                DisplayUtility.showKeyboard(searchField);
            }
            if (listener != null) {
            ...
            searchContainer = new FrameLayout(getContext()) {
                ...
                protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                    ...
                    if (searchFieldCaption.getVisibility() == VISIBLE) {
                        ...
                        width = searchFieldCaption.getMeasuredWidth() + DisplayUtility.dp(4);
                    } else {
                        width = 0;
                    }
                ...
                protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
                    ...
                    if (LocaleController.isRTL) {
                        ...
                        if (searchFieldCaption.getVisibility() == VISIBLE) {
                            x = searchFieldCaption.getMeasuredWidth() + DisplayUtility.dp(4);
                        } else {
                            x = 0;
                        }
                    }
            ...
            searchField = new EditTextBoldCursor(getContext()) {
                ...
                public boolean onTouchEvent(MotionEvent event) {
                    if (event.getAction() == MotionEvent.ACTION_DOWN) {
                        if (!DisplayUtility.showKeyboard(this)) {
                            clearFocus();
                            requestFocus();
                        }
                    }
    ...
    private void updateOrShowPopup(boolean show, boolean update) {
        ...
        if (parentMenu != null) {
            ...
            if (subMenuOpenSide == 0) {
                ...
                if (show) {
                    popupWindow.showAsDropDown(parent, getLeft() - DisplayUtility.dp(8) + (int) getTranslationX(), offsetY);
                }
                if (update) {
                    popupWindow.update(parent, getLeft() - DisplayUtility.dp(8) + (int) getTranslationX(), offsetY, -1, -1);
                }
            }
        } else {
            if (subMenuOpenSide == 0) {
                ...
                if (show) {
                    popupWindow.showAsDropDown(this, -DisplayUtility.dp(8) + additionalXOffset, offsetY);
                }
                if (update) {
                    popupWindow.update(this, -DisplayUtility.dp(8) + additionalXOffset, offsetY, -1, -1);
                }
            } else {
                if (show) {
