...
public class DrawerLayoutContainer extends FrameLayout {
    ...
    private PresenterLayout parentPresenterLayout;
    ...
    public DrawerLayoutContainer(Context context) {
        ...
        minDrawerMargin = (int) (MIN_DRAWER_MARGIN * DisplayUtility.density + 0.5f);
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            setOnApplyWindowInsetsListener((v, insets) -> {
                ...
                if (DisplayUtility.statusBarHeight != insets.getSystemWindowInsetTop()) {
                    drawerLayout.requestLayout();
                }
                DisplayUtility.statusBarHeight = insets.getSystemWindowInsetTop();
                lastInsets = insets;
    ...
    public void openDrawer(boolean fast) {
        ...
        if (DisplayUtility.isTablet() && parentPresenterLayout != null && parentPresenterLayout.parentActivity != null) {
            DisplayUtility.hideKeyboard(parentPresenterLayout.parentActivity.getCurrentFocus());
        }
    ...
    public void setParentPresenterLayout(PresenterLayout layout) {
        parentPresenterLayout = layout;
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (!parentPresenterLayout.checkTransitionAnimation()) {
            ...
            if (allowOpenDrawer && parentPresenterLayout.fragmentsStack.size() == 1) {
                if (ev != null && (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_MOVE) && !startedTracking && !maybeStartTracking) {
                    ...
                    if (maybeStartTracking && !startedTracking && (dx > 0 && dx / 3.0f > Math.abs(dy) && Math.abs(dx) >= DisplayUtility.getPixelsInCM(0.2f, true) || drawerOpened && dx < 0 && Math.abs(dx) >= Math.abs(dy) && Math.abs(dx) >= DisplayUtility.getPixelsInCM(0.4f, true))) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        return parentPresenterLayout.checkTransitionAnimation() || onTouchEvent(ev);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (Build.VERSION.SDK_INT < 21) {
            inLayout = true;
            if (heightSize == DisplayUtility.displaySize.y + DisplayUtility.statusBarHeight) {
                if (getLayoutParams() instanceof MarginLayoutParams) {
                    setPadding(0, DisplayUtility.statusBarHeight, 0, 0);
                }
                heightSize = DisplayUtility.displaySize.y;
            } else {
                if (getLayoutParams() instanceof MarginLayoutParams) {
                    setPadding(0, 0, 0, 0);
                }
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (drawingContent) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                final int vright = (int) v.getX() + v.getMeasuredWidth();
                if (vright > clipLeft) {
                    clipLeft = vright;
                }
            }
        ...
        if (scrimOpacity > 0 && drawingContent) {
            ...
            final float alpha = Math.max(0, Math.min(drawerPosition / DisplayUtility.dp(20), 1.0f));
            if (alpha != 0) {
